...
5. **人工 ACG 資料必須保留來源與決策，但不同層有不同生命週期。** `acg_events.json`、metadata／admission overrides、`venue_address_overrides.json`、`venue_geocodes.json` 與 `data/manual/_kv_cache/` 是持久 SSOT；`manual_extra.json`、`public/venues.json` 與 `public/kv/` 是依 Asia/Taipei 當日有效活動重建的衍生層，`endDate < 今天` 的活動會正常略過，最後一個引用圖釘到期後的孤兒公開 KV 副本也會被清理。管線、報表與 Daily Update PR 日期都必須使用 Asia/Taipei，不得受 GitHub runner UTC 或本機時區影響。不得為保留歷史、座標或公開 KV 副本而要求到期活動繼續留在衍生輸出。來源：`專案交接文件.md` §3 第四層、§10；現行細則見根目錄 `decision.md`「場館資料生命週期與 geocode SSOT」及「排程與活動生命週期的標準時區」。
   六館動態清單即使非空也可能只載入第一批；駁二新清單若缺少昨日仍在官方展期內的活動，必須保留本次新抓資料並補回昨日仍有效的缺項，同時記錄 `PARTIAL`。已到期活動不可補回，仍依上述生命週期正常下架。
   六館 generated 中的非 ACG 稽核資料會隨官方清單增減，測試只能保護來源、日期格式與公開邊界，不得把某一筆非 ACG 活動的永久存在寫成契約；列表標題若退化為日期片段，應以同一官方詳情頁的結構化標題補正。
