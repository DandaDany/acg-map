...
```
目標：     整合 2026/09/02 Daily Update #125，修正松山相鄰卡日期污染與駁二 AJAX／詳情日期退化。
...
已完成：   接手成功的 run #58／PR #125 最新快取；保留華山新增兩筆非 ACG 稽核資料與柯南咖啡到期下架。
           松山改以活動連結自身為卡片邊界；駁二等待 AJAX 完整清單並從詳情頁結構化展期欄位取日期。
           修復《獵人實境解謎》誤消失、漫畫便當店空日期與兩筆松山單日活動誤延長，並補日期感知回歸測試。
目前錯誤： 無資料或測試錯誤；本地縮圖工具的 3 個既有 SVG 因未安裝 Playwright 無法重繪，不影響本輪公開活動或既有縮圖。
下一步：   建立單一整合 PR supersede #125，等待 CI 與 Daniel 明確同意；下一次 Daily Update 驗證正式六館爬蟲。
不要改動： 未經 Daniel 明確同意不得合併；不得以部分清單覆蓋完整駁二資料，不得覆蓋人工 SSOT、座標、KV、費用與 UI 決策。
```
