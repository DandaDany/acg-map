...
步驟：
 ...
 13) report_event_kv.py  依最終 venues.json 重產缺口清單（KV／缺連結／約略定位）供人看與機器讀
...
run("report_event_kv.py")  # 依最終 venues.json 重產缺口報表：_missing_event_kv.json（機器讀）＋ docs/資料缺口清單.md（人看：KV破圖/待補＋缺連結＋約略定位）
