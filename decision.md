...
- `public/kv/` 與公開活動採相同生命週期：最後一個引用圖釘到期後，未被其他活動引用的公開 KV 副本應由管線清理。人工來源列與 `data/manual/_kv_cache/` 的持久 KV 仍須保留；回歸測試不得把已到期活動的孤兒 `public/kv/` 副本誤當成永久 SSOT。
...
- 動態清單的「非空」不代表完整。駁二若新清單缺少昨日仍在官方展期內的活動，必須視為 AJAX 部分載入；保留本次新抓資料並從昨日 generated 補回仍有效的缺項，在 log 明確標示 `PARTIAL`。已到期活動不可補回，仍須正常下架。不得以降低活動存在性測試或接受較少筆數來掩蓋來源退化。
...
- 六館 `generated` 中尚未進入公開地圖的非 ACG 活動是每日稽核快照，不是永久資料契約。回歸測試可驗證官方來源、日期格式及「不得漏入公開層」，但不得要求某一筆非 ACG 活動永遠存在；詳情頁已公開的結構化標題與日期優先於列表卡的日期碎片。
