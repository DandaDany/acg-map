...
- 圖片 marker 的 KV 必須置中完整呈現，剩餘空間以同圖模糊背景補齊；活動數徽章固定在圖片內右上角，使用紅底白字。
- Marker 顯示模式切換固定放在地圖上方正中央。
- 點擊圖釘或圖片 marker 時，地圖必須同步移動並定位至該 marker 座標，再顯示活動資訊。
...
- 同活動有多個地點時，詳情卡先顯示目前定位的主要地點，其他地點區標示「其他活動地點」。詳情卡本身固定高度，其他地點一次最多露出三筆，超出的地點只在該區塊內捲動。
- 地點列只保留無外框的「導航」文字連結，不顯示 Threads 圖示。
